...
- `daemon`: zone daemon process (owns all brainCli PTYs in the zone)
- `host`: `local` | `cloud`
...
- the brain slug must reference a supplier with BrainCli support; clones require a CLI process that khlone can manage via PTY. the slug format is defined by rhachet and the brain suppliers.
...
khlone runs **one daemon per zone**. the daemon owns all brainCli PTY processes in that zone — it is the session manager that eliminates the need for tmux, screen, or any external terminal multiplexer.

```
zone (@feat/auth)
└── daemon (pid: 1000)
    ├── brainCli: foreman.1 (pid: 1001)    [process group: foreman.1]
    ├── brainCli: mechanic.1 (pid: 1002)   [process group: mechanic.1]
    └── brainCli: researcher.1 (pid: 1003) [process group: researcher.1]
```
...
| persist sessions | daemon owns the PTY — survives shell exit |
| attach/detach | `khlone talk` attaches, `/exit` detaches |
| stream output | `khlone watch` reads brainCli stdout |
| dispatch tasks | writes to brainCli PTY stdin |
| crash recovery | catches `SIGCHLD`, respawns via BrainSeries continuation |
...
**crash isolation invariant:** a clone crash must never take down the daemon or peer clones. each clone's brainCli runs in its own process group — the daemon catches crashes per child and restarts just that clone via BrainSeries continuation (new episode, same series).
...
| **tmux for ai** | daemon owns PTY sessions — persist, attach, detach — no tmux needed |
...
| headless efficiency | no ui overhead, daemon owns PTY |
...
| per-zone daemon | owns all brainCli PTYs, eliminates tmux dependency |
| find-or-enroll brainCli | reuse extant or spawn fresh via rhachet |
| clone crash isolation | process groups per clone, daemon catches and restarts |
| queue persistence | survives shell exit — daemon owns processes |
| crash recovery | auto-restart via BrainSeries continuation |
| headless execution | daemon-managed PTY, low resource |
...
- **brains via rhachet** — all clones use `BrainCli` contract (CLI + PTY) from `rhachet-brains-*` suppliers
...
- **per-zone daemon** owns all brainCli PTYs — eliminates tmux, enables persist/attach/detach
...
- **clone crash isolation** via process groups — one crash never takes down peers
- **headless execution** via daemon-managed PTY
