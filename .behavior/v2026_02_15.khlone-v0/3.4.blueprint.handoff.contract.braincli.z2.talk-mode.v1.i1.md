...
the handoff contract assumes khlone can switch a clone between headless dispatch (`ask`/`act`) and interact talk mode. the z1 zoomin showed that no CLI supports live mode-switch on a process — but `enboot`/`reboot` hides this behind the BrainCli handle.
...
2. **the daemon controls the full exit lifecycle.** any brain CLI exit, any CLI disconnect, any crash — the daemon detects it and reboots to dispatch. no "signal back to the CLI so it can tell the daemon" round-trip. the daemon is the authority on process state.
...
4. any brain CLI exit triggers reboot to dispatch mode
5. any CLI disconnect (ctrl+c, terminal close) triggers reboot to dispatch mode
...
2. khlone CLI -> daemon (IPC): { type: 'talk', cloneSlug: 'foreman.1' }
   daemon:
     a. call brainCli.reboot({ mode: 'interact' })
...
3. khlone CLI receives talk-ready
   khlone CLI:
     ...
     b. begin relay loop:
        - process.stdin -> IPC { type: 'input', data } -> daemon -> brainCli.write(data)
        - brainCli.onData -> daemon -> IPC { type: 'output', data } -> khlone CLI -> process.stdout

4. user is now in the brain CLI's TUI/REPL
   - keystrokes: user terminal -> khlone CLI -> IPC -> daemon -> brainCli PTY
   - output: brainCli PTY -> daemon -> IPC -> khlone CLI -> user terminal
...
5. talk mode ends (any of these triggers):
   a. brain CLI exits (crash, /exit, normal end)
      -> brainCli.onExit fires in daemon
      -> daemon reboots to dispatch: brainCli.reboot({ mode: 'dispatch' })
...
4. daemon calls `brainCli.resize(cols, rows)` — sets the PTY dimensions
...
interface BrainCli {
  ...
  // lifecycle
  enboot(input: { mode: 'dispatch' | 'interact' }): Promise<void>;
  reboot(input: { mode: 'dispatch' | 'interact' }): Promise<void>;
  kill(): void;
...
- `write()` is valid in both modes — in `dispatch` mode it writes to the structured stdin; in `interact` mode it writes raw PTY input
- `resize()` is valid in both modes — primarily used in `interact` mode for SIGWINCH proxy
- `onData` fires in both modes — in `dispatch` mode it emits structured events; in `interact` mode it emits raw terminal output
- `reboot()` can be called in either mode — it always kills the current process and respawns in the target mode

**no `standby` mode.** the daemon always has a live process or is in the middle of a reboot. the handle manages the reboot internally — khlone never sees a gap.

**no `prepareInteract` / `completeInteract`.** those were artifacts of the direct-connect model where the CLI spawned the interact process. with daemon-owned byte relay, the daemon handles the full mode switch via `reboot()`.
...
khlone CLI                              daemon
    ...
    |                                     |
    |  { type: 'input', data: '...' }    |  ----> brainCli.write(data)
    | ----------------------------------> |
    |                                     |
    |  { type: 'output', data: '...' }   |  <---- brainCli.onData
    | <---------------------------------- |
    |                                     |
    |  { type: 'resize', cols, rows }    |  ----> brainCli.resize(cols, rows)
    | ----------------------------------> |
    |                                     |
    |  ... byte relay continues ...       |
    |                                     |
    |  [brain CLI exits]                  |  <---- brainCli.onExit fires
...
1. `brainCli.onExit` fires in the daemon
2. daemon reboots to dispatch: `brainCli.reboot({ mode: 'dispatch' })`
3. daemon sends `{ type: 'talk-end', reason: 'brain-exit', code }` to khlone CLI
4. khlone CLI restores terminal, informs the user: "brain CLI exited. clone rebooted to dispatch mode."
5. daemon resumes task queue

the daemon handles crash recovery the same way regardless of mode — `onExit` fires, reboot to dispatch.

### what if the khlone CLI crashes or disconnects?

1. the IPC connection drops
2. daemon detects client disconnect
3. daemon reboots to dispatch: `brainCli.reboot({ mode: 'dispatch' })`
...
| mode | direction | raw terminal? | daemon reboots mode? |
|---|---|---|---|
...
**watch IPC:**
```
brainCli.onData -> daemon -> IPC { type: 'output' } -> khlone CLI -> process.stdout
```

**talk IPC:**
```
process.stdin -> khlone CLI -> IPC { type: 'input' } -> daemon -> brainCli.write()
brainCli.onData -> daemon -> IPC { type: 'output' } -> khlone CLI -> process.stdout
```
...
| who spawns the interact process? | the daemon (via `brainCli.reboot({ mode: 'interact' })`) |
| what does the khlone CLI do in talk mode? | thin relay: set raw mode, pipe stdin -> IPC -> daemon, pipe daemon -> IPC -> stdout |
| how does talk mode end? | any brain CLI exit OR any CLI disconnect -> daemon reboots to dispatch |
| how does SIGWINCH work? | khlone CLI sends resize IPC message, daemon calls brainCli.resize() |
...
talk mode:    user terminal <-> khlone CLI <-> IPC <-> daemon <-> brainCli (interact)
              (bidirectional relay, daemon reboots to interact then back to dispatch)
```

### what this changes in the handoff contract

1. **`reboot({ mode: 'interact' })` is the mechanism** — the daemon reboots the brain CLI to interact mode for talk
2. **`reboot({ mode: 'dispatch' })` on any exit** — brain CLI exit or CLI disconnect triggers reboot back to dispatch
...
5. **`resize(cols, rows)` added to handle** — daemon proxies terminal size for interact mode
