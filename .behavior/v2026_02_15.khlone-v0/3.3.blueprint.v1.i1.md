...
**note on file count vs. decomposition:** the file tree above declares **184 files** (all v0 files). the decomposition (`3.5.decomposition.v1.i1.md`) counts **177 files** across its eight blocks. the gap of 7 files comes from utility operations and test assets that are implicit dependencies of named block deliverables but not separately called out in block scopes. the decomposition is the implementation roadmap; this table is the complete file inventory.
...
[+] daemon entry (spawned by setZoneDaemon)
  ...
  │       ├── [+] 'attach'               → pipe brainCli.terminal.onData to client (watch)
  │       ├── [+] 'detach'               → stop pipe (watch or talk)
  │       ├── [+] 'talk'                 → brainCli.executor.boot({ mode: 'interact' }), begin byte relay
  │       ├── [+] 'talk-end'             → sent to client when brain CLI exits or detach
  │       ├── [+] 'input'                → brainCli.terminal.write(data) (talk mode input relay)
  │       ├── [+] 'resize'               → brainCli.terminal.resize(cols, rows) (SIGWINCH proxy)
  ...
  │   ├── [+] brainCli.executor.boot({ mode: 'dispatch' })  # initial boot (headless)
  │   ├── [+] brainCli.terminal.onExit → crash handler
  │   │   ├── [+] daoCloneCheckpoint.set.append       # access: save checkpoint
  │   │   └── [+] brainCli.executor.boot({ mode: 'dispatch' })  # respawn via same handle, series continued
  │   └── [+] brainCli.terminal.onData → transcript capture
...
[+] invokeTalk
  ...
  └── [+] setTerminalToTalkWithClone({ clone })
      ├── [+] IpcMessage { type: 'talk', cloneSlug }     # daemon boots to interact, begins relay
      ├── [+] set process.stdin to raw mode                # no local echo, no line buffer
      ├── [+] relay: process.stdin → IPC { type: 'input' } → daemon → brainCli.terminal.write()
      ├── [+] relay: brainCli.terminal.onData → daemon → IPC { type: 'output' } → process.stdout
      ├── [+] proxy: process.stdout resize → IPC { type: 'resize' } → brainCli.terminal.resize()
      ├── [+] on IpcMessage { type: 'talk-end' } → restore terminal, return to shell
      └── [+] on SIGINT → IpcMessage { type: 'detach' }  # daemon boots to dispatch
...
```
[+] brainCli.terminal.onExit fires (unexpected exit)
  ├── [+] identify crashed clone by handle ref
  ├── [+] mark clone status → 'idle' (in-memory; daoClone.set.upsert after block F)
  ├── [+] capture checkpoint (in-memory; daoCloneCheckpoint.set.append after block F)
  ...
  ├── [+] update clone ref (in-memory; daoClone.set.upsert after block F)
  └── [+] re-dispatch active task from checkpoint
```

**note:** this codepath is split across two decomposition blocks:
- **block C (clone-level, stateless):** the daemon detects exit, reads series ref from the in-memory handle, boots a fresh process, and re-dispatches. no DAOs, no disk writes. the daemon is alive and holds all state.
- **block F (daemon-level, persisted):** adds `daoCloneCheckpoint` and `daoClone` persistence so that crash recovery survives daemon death. the logic is the same — block F wraps the in-memory path with disk writes.

crash recovery and talk-mode exit both use `brainCli.executor.boot({ mode: 'dispatch' })`. in talk mode, the daemon detects brain CLI exit (or CLI disconnect) and boots to dispatch — the same `executor.boot` path as crash recovery. the mechanism is unified.
...
| `rhachet` | `1.33.0` | [○] retain (dev) — actor enrollment, brain slug → BrainCli handle (executor.boot/ask/act/terminal.write/terminal.resize/terminal.onData/terminal.onExit) |
...
domain           business logic lives here — all get/set/gen operations
                 depends on: domain.objects (shapes), access/daos (persistence), infra (ipc/fs)
...
infra            domain-free utilities — ipc, filesystem, process
...
| observe/ | setTerminalToWatchClone receives real BrainCli output stream |
...
| domain.operations (i/o) | integration | ~31 | clone enroll, zone init, daemon spawn/connect, dispatch via ipc, observe via BrainCli, orchestrator persistence |
| access/daos | integration | ~8 | json/jsonl read/write/append round-trip per dao |
| infra | integration | ~9 | ipc server/client, filesystem json/jsonl, process spawn/pid/alive, gitroot/branch |
...
1. infra/ipc — createIpcServer, createIpcClient, IpcMessage
2. daemon/ — setZoneDaemon, getOneZoneDaemon, genZoneDaemon, getOneDaemonConnection
3. daemon/ — genCloneViaDaemon, setCloneViaDaemon (crash recovery)
4. dispatch/ — dispatchTask, enqueueTask, disruptClone, dispatchSkill
...
| terminal output parse complexity | medium | medium | daemon streams raw output via BrainCli; transcript captures as-is; no parse required for v0 |
...
khlone v0 is a greenfield implementation of **176 files** across 5 layers:

- **infra** (leaf) — ipc, filesystem, process utilities (BrainCli abstracts PTY away)
...
the execution flows bottom-up across 6 phases, with each phase testable in isolation before the next begins. the daemon (phase 3) is the most complex subsystem — it holds BrainCli handles per clone, dispatches tasks via `.ask()`/`.act()`, and uses `.executor.boot()` for crash recovery and talk mode. talk mode uses a byte relay model: the daemon calls `brainCli.executor.boot({ mode: 'interact' })` and relays raw terminal bytes between the IPC client (khlone CLI) and the brain CLI. any brain CLI exit or CLI disconnect triggers `brainCli.executor.boot({ mode: 'dispatch' })`.

all brain interaction flows through rhachet's BrainCli contract (`executor.boot`/`ask`/`act`/`terminal.write`/`terminal.resize`/`terminal.onData`/`terminal.onExit`). khlone never spawns CLI processes directly — the daemon owns all brain CLI processes in both dispatch and interact modes, and the khlone CLI is always a thin relay. all persistence lives under `~/.khlone/` as json/jsonl files — no database required for v0.
