...
**quote:**
> "role init: `--who role++` spawns a clone with the specified role alias. alias resolves to fully qualified name (e.g., `mechanic` → `ehmpathy/mechanic`). rhachet knows how to init each role (briefs, skills, hooks)."
...
**quote:**
> "skill (template from rhachet) └── Task (instance when --skill used)"
...
- two action modes: ask (read-only), act (read-write) — see lesson.13
- no separate `spawnBrain()` — brains are composed into actors
...
**quote:**
> "role (template from rhachet) └── Clone (instance) ├── adds: status, pid, queue, progress └── adds: brain (selected from config)"
...
**quote:**
> "if hero knows the skill → hero gets it. if exactly one role knows it → that role gets it (enroll if needed). if multiple roles know it → failfast, ask for disambiguation. if zero roles know it → failfast, inform user."
...
**quote:**
> "first `khlone ask` or `khlone act` triggers implicit `khlone init`: 1. read config — find `khlone.worksite.yml` at gitroot 2. resolve worktree — detect branch + worktree path 3. bind worksite — create `.khlone/.bind.worksite/` in worktree 4. spawn clone.0 — start default clone per crew config"
...
- `actor.ask()` = read-only — queries, explanations, research (no mutations)
- `actor.act()` = read-write — implementations, modifications, file changes

**solid/rigid/fluid is a RoleSkill concern:**
the three grains of determinism still exist, but at the **skill registration** level:
- 🪨 solid skill — deterministic, no brain (e.g., `lint.fix`, `test.run`)
- 🔩 rigid skill — harness controls flow, brain augments (e.g., `review.pr`)
- 💧 fluid skill — probabilistic, brain decides path (e.g., `research.topic`)
...
// atoms: BrainAtom[] — single-turn inference
// repls: BrainRepl[] — multi-turn agentic loops
```
