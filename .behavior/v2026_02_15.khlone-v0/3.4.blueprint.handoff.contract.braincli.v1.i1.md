...
- the process must support PTY — khlone uses node-pty to manage it
...
**why:** crash recovery is a core v0 feature. the daemon catches `SIGCHLD`, identifies which clone crashed, saves a checkpoint, and respawns via BrainSeries continuation.
...
- must clean up any child processes the CLI may have spawned (process group kill)
...
khlone uses node-pty today. whether rhachet:
...
**preference:** khlone would prefer to own the PTY spawn (rhachet returns the command + args + env, khlone calls node-pty). this keeps khlone in control of process groups, which it needs for crash isolation. but if rhachet has good reasons to own the PTY, khlone can adapt.
...
```ts
// daemon: foreman.1's brainCli process died (SIGCHLD)
...
async function handleCloneCrash({ cloneSlug, code, signal }) {
  ...
  // 4. re-register hooks
  brainCli.onData((chunk) => { /* transcript */ });
  brainCli.onExit((code, signal) => { handleCloneCrash({ cloneSlug, code, signal }); });
...
//    daemon pipes brainCli.onData to IPC client
//    (brainCli.onData was already registered — daemon just adds a second consumer)
...
// brainCli.onData still fires for transcript — only the IPC pipe stops
```
...
// daemon: calls brainCli.reboot({ mode: 'interact' })
...
//    a. brain CLI exits -> daemon catches onExit, reboots to dispatch, sends talk-end
//    b. khlone CLI disconnects (ctrl+c, terminal close) -> daemon detects IPC drop, reboots to dispatch
//    c. user sends detach signal -> CLI sends { type: 'detach' }, daemon reboots to dispatch
```
...
├── PTY node-pty management (preferred)   ├── BrainOutput metrics capture
...
| talk mode (interactive) | yes — no `reboot({ mode: 'interact' })` for byte relay |
