...
> **note:** z2 (`3.4.blueprint.handoff.contract.braincli.z2.talk-mode.v1.i1.md`) builds on this doc. z2 establishes a daemon-owned byte relay model for talk mode: the daemon calls `brainCli.executor.boot({ mode: 'interact' })` and relays raw PTY bytes between the IPC client and the brain CLI. the `executor.boot` model from this doc is retained — z2 adds the interact mode and byte relay protocol. the per-CLI research findings in this doc remain valid.
...
### the boot approach: `executor.boot`
...
the insight: `brainCli.executor.boot({ mode })` kills the current OS process, captures the series ref, and respawns in the new mode — all behind the same handle. khlone never sees the process churn. it just sees a mode change.

**one unified method:**
- `brainCli.executor.boot({ mode: 'dispatch' })` — initial boot (first spawn) or mode switch (kill + respawn with same series)
...
brainCli.executor.boot({ mode: 'dispatch' })
  ...
  → handle is live, pid updated, terminal.onData/terminal.onExit re-wired

brainCli.executor.boot({ mode: 'interactive' })
  ...
  → handle stays the same, pid updated, terminal.onData/terminal.onExit re-wired
...
| concern | who owns it |
|---|---|
| "I need this clone in talk mode now" | khlone (calls `executor.boot({ mode: 'interactive' })`) |
...
khlone's daemon never needs to know how the mode switch works. it just calls `executor.boot` and the handle comes back alive in the new mode. the series ref is the bridge — the supplier uses it to ensure the new process continues where the old one left off.

### the clone lifecycle with executor.boot

```
khlone act "implement auth"
  → daemon calls brainCli.executor.boot({ mode: 'dispatch' })
  → daemon calls brainCli.act({ prompt: 'implement auth' })
  → brainCli.terminal.onData fires — daemon captures output

khlone act "add tests"
  → daemon calls brainCli.act({ prompt: 'add tests' })      (same handle, no boot needed)

khlone talk
  → daemon calls brainCli.executor.boot({ mode: 'interactive' })   (supplier kills + respawns)
  → daemon pipes user terminal ↔ brainCli terminal (write/onData)
  → user types /exit
  → daemon calls brainCli.executor.boot({ mode: 'dispatch' })  (back to headless)

khlone act "task 3"
  → daemon calls brainCli.act({ prompt: 'task 3' })         (same handle, series continued)
```

**the handle is stable across boots.** khlone holds one `brainCli` ref per clone for the clone's entire lifetime. the pid changes on each boot, the mode changes, but the handle identity is stable. `terminal.onData` and `terminal.onExit` callbacks are re-wired automatically by the supplier after each boot.

### crash recovery fits naturally

crash recovery is just a boot that khlone triggers instead of the user:

```
brainCli.terminal.onExit fires (unexpected crash)
  → daemon calls brainCli.executor.boot({ mode: 'dispatch' })
...
`brainCli.terminal.onData` fires in both `dispatch` and `interactive` modes. the daemon captures all output regardless of mode. `khlone watch` just forwards the terminal.onData stream to the user's terminal.
...
interface BrainCli {
  ...
  // lifecycle
  executor: {
    boot(input: { mode: 'dispatch' | 'interactive' }): Promise<void>;
  };
  kill(): void;
...
- `terminal.write()` is valid in both modes — in `dispatch` mode it writes to the structured stdin; in `interactive` mode it writes raw PTY input
- `terminal.onData` fires in both modes — in `dispatch` mode it emits structured events; in `interactive` mode it emits raw terminal output
...
| question | answer |
|---|---|
| can you switch a live CLI process between interactive and headless? | no — but `executor.boot` hides this behind the BrainCli abstraction |
| can you dispatch tasks headless? | yes — all three CLIs support it via `dispatch` mode |
| can you continue sessions across boots? | yes — all three via series ref (session ID) |
| how does talk mode work? | `brainCli.executor.boot({ mode: 'interactive' })` — same handle, new process |
| how does watch mode work? | `brainCli.terminal.onData` fires in both modes — daemon forwards to user |
| how does crash recovery work? | `brainCli.executor.boot({ mode: 'dispatch' })` — same as any other boot |
...
clone lifecycle:
  ...
  brainCli.executor.boot({ mode: 'dispatch' })          # initial spawn
  brainCli.act({ prompt: 'implement auth' })          # task 1
  brainCli.act({ prompt: 'add tests' })               # task 2 (same process)
  brainCli.executor.boot({ mode: 'interactive' })            # talk mode (kill + respawn)
  ... user interacts via PTY ...
  brainCli.executor.boot({ mode: 'dispatch' })           # back to headless (kill + respawn)
  brainCli.act({ prompt: 'fix lint' })                # task 3 (series continued)
  [crash] → brainCli.executor.boot({ mode: 'dispatch' }) # auto-recovery (same as any boot)
...
1. **§1.1 (spawn):** replace `genBrainCli` with handle creation + `brainCli.executor.boot({ mode })` — the handle is created first, then booted
2. **§1.2 (ask/act dispatch):** add mode constraint — only valid in `dispatch` mode
3. **§1.5 (crash detection):** crash recovery is just `executor.boot({ mode: 'dispatch' })` — same mechanism as talk→dispatch transition
4. **§1.8 (BrainSeries):** series is internal to the handle — khlone can read it but doesn't need to pass it on boot. the handle manages its own series across boots.
5. **§4.5 (handle shape):** updated with `executor.boot`, `mode`, and mode constraints on `ask`/`act`
6. **no separate `genBrainCliInteractive` needed** — `executor.boot({ mode: 'interactive' })` covers it
