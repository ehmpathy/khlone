...
- their brainCli process (the PTY daemon) stays alive between tasks
...
7. rhachet returns brainCli instance (PTY process, pid: 12345)
...
**key:** no rhachet call. the brainCli is alive and reused. the PTY process persists.
...
6. khlone calls rhachet to spawn a new brainCli from slug
7. rhachet returns brainCli instance (PTY process, pid: 12346)
...
6. rhachet returns grokBrainCli (PTY process, pid: 12347)
...
6. rhachet returns new brainCli (PTY process, pid: 12349)
...
| session persists after shell exit | brainCli persists — daemon owns the PTY |
| `tmux attach -t sess` | `khlone talk` attaches to clone's brainCli |
| `ctrl+b d` (detach) | `/exit` detaches, clone continues |
| `tmux send-keys "cmd"` | daemon writes to brainCli PTY stdin |
...
this is implemented via node-pty (native node PTY binds) rather than shell-out to tmux, which avoids the external dependency and the overhead of tmux output parse.
...
- each clone's brainCli runs in its own process group for crash isolation
...
- each clone's brainCli runs in its own process group
- daemon catches `SIGCHLD` per child — a crash in foreman.1's brainCli is isolated to that process group
