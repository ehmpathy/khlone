...
|-------------------|----------------|--------------------|--------------------|-----------|------------------|
| clone crashes | daemon detects via `brainCli.terminal.onExit` | auto-restart via BrainSeries (new episode, same series) | unaffected (separate brainCli handle isolation) | resume from checkpoint | none |
| shell exits | n/a (daemon owns brainCli handle) | continues (daemon-owned) | continues | queue preserved | none |
...
**note:** clone crash isolation is enforced via separate brainCli handles — each clone has its own brainCli handle (separate process). the daemon detects crashes via `brainCli.terminal.onExit` per clone and restarts just that clone. BrainSeries continuation ensures the new brainCli resumes within the same series (new episode, same series), so cross-context memory persists.
